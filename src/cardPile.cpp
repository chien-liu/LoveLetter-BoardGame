...
namespace loveletter
{
  CardPile::CardPile(int n_player) : top(0)
  {
    if (n_player < 2 || n_player > 4)
      throw std::runtime_error("CardPile::CardPile(int n_player), more than 4 players.");
    ...
    if (n_player == 2)
    { // Eliminate two cards from the deck
      for (int i = 0; i < 2; i++)
      {
        delete pile[i];
      }
  ...
  {
    if (top >= pile.size())
      throw std::runtime_error("Card CardPile::next(), attempt to draw an empty CardPile.");
    return pile[top++];
  }

  Card* const CardPile::GetReservedCard() const
  {
