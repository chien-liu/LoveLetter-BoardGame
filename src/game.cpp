...
namespace loveletter
{
  ...
  void Game::drawCard()
  {
    players.current()->drawCard(pile.next());
  }
  ...
  void Game::update()
  {
    ...
    case 1:
      std::cout << "[Game Info] Target Player " << targetPlayer->name
                << std::endl
                << "[Game Info] Guess card " << assassinatedCardNumber << std::endl;
  ...
  void printPlayers(const Game &game)
  {
    ...
    if (typeid(*game.players.current()) == typeid(Human))
    { // Human Player
      std::cout << "# of cards remaining in the pile: " << game.pile.count() << std::endl;
    }
